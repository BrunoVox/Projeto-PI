...
img=imread('E:\DevProj\_Datasets\MiscData\Barbara.png');
if (size(img,3)==3)
...
subplot(2,2,3);displayPatches(dict);colormap(gray);title('Learned dictionary'); 
pause(0.05)
...
params.dict=dict; 
params.stride=1;
params.beta=0.01; % weight of the reconstructed image
...
subplot(2,2,4);imagesc(img_denoise,[0 255]);colormap(gray);title('Denoising image')
