...
def PatchExtraction(img, patch_size, pad_flag, crop_type, *args):
    ...
    if pad_flag == 1:
        img_pad = np.pad(img, (patch_size_half, patch_size_half), 0, "reflect")
    [nrow, ncol, nchannel] = np.shape(img_pad)[0], np.shape(img_pad)[1], np.shape(img_pad)[2]
